...
//                        mTestResults[ct] = r.totalTime / r.iterations * 1000.f;
                        mTestResults[ct] = (r.iterations / r.totalTime )* 1.f;
                        mTestInfo[ct] = r.testInfo;
                    }
                    mRun = false;
                    onBenchmarkFinish(!mRun);
                    // total batch processed
//                    if(batchCounter >= mBatchSize){
//                        mRun = false;
//                        onBenchmarkFinish(true);
//                    }
                } else {
                    // Run the kernel
//                    runTest();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT < 30) {
            ...
            Log.d(TAG, "Preferred preference leo: " + com.mediatek.neuropilot_R.Platform.getPreferredInference());
        }
    }
...
//            mProcessor.exit();
    ...
    protected void onResume() {
        ...
        mBatchSize = i.getIntExtra("batchSize", 1);
...
//            mProcessor.start();
            for(int loop = 0 ; loop < mBatchSize ; loop++){
                Processor mtProcessor = new Processor(!mDemoMode);
                mtProcessor.start();
            }
        }
    }
